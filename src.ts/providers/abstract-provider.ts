...
export class AbstractProvider implements Provider {
  ...
  >(
    ...
    console.log(
      "Handling Valid CCIP... CCIP Result:",
      ccipResult,
      "New TX Data:",
      tx.data
    );
  ...
  async estimateGas(
    ...
    try {
      ...
      return await this.#handleValidCCIP(
        ...
        attempt,
        "estimateGas",
        async (newTxn: TransactionRequest, newAttempt: number) =>
          await this.estimateGas(newTxn, newAttempt)
      );
    }
  }

  async resolveOffchainData(
    _tx: TransactionRequest,
    attempt: number = 0
  ): Promise<string> {
    ...
    try {
      ...
      return await this.#handleValidCCIP(
        ...
        "estimateGas",
        async (newTxn: TransactionRequest, newAttempt: number) =>
          await this.resolveOffchainData(newTxn, newAttempt)
      );
    }
  }
