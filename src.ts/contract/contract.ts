...
interface ContractRunnerOffchainResolver extends ContractRunner {
...
function canResolveOffchaindata(
  value: any
): value is ContractRunnerOffchainResolver {
...
function buildWrappedFallback(contract: BaseContract): WrappedFallback {
  ...
  const resolveOffchainData = async function (
    overrides?: Omit<TransactionRequest, "to">
  ): Promise<string> {
    const runner = getRunner(contract.runner, "resolveOffchainData");
    assert(
      canResolveOffchaindata(runner),
      "contract runner does not support resolving offchain data",
      "UNSUPPORTED_OPERATION",
      { operation: "resolveOffchainData" }
    );

    return await runner.resolveOffchainData(
      await populateTransaction(overrides)
    );
  };
