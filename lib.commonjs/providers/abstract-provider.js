...
class AbstractProvider {
    ...
    #isValidOffchainLookup(error, transaction, attempt, type = "read", blockTag) {
        if (!this.disableCcipRead &&
            ...
            (0, index_js_6.isCallException)(error) &&
            error.data &&
            attempt >= 0 &&
            blockTag === "latest" &&
            transaction.to != null &&
            (0, index_js_6.dataSlice)(error.data, 0, 4) === "0x556f1830")
            return true;
        return false;
    }
    /**
    ...
    async ccipReadFetch(tx, calldata, urls) {
        ...
        const sender = typeof tx.to == "string"
            ? tx.to.toLowerCase()
            : (await (0, index_js_1.resolveAddress)(tx.to, this)).toLowerCase();
        ...
        (0, index_js_6.assert)(false, `error encountered during CCIP fetch: ${errorMessages
    ...
    async resolveOffchainData(_tx, attempt = 0) {
        ...
        try {
            if (isPromise(tx))
                tx = await tx;
            (0, index_js_6.getBigInt)(await this.#perform({
                method: "estimateGas",
                transaction: tx,
            }), "%response");
            // Gas estimation went through w/ current TX data === PASS
            return (0, index_js_6.hexlify)(tx.data);
        }
        catch (error) {
            // CCIP Read OffchainLookup
            if (!this.#isValidOffchainLookup(error, _tx, attempt, "write", _tx.blockTag))
